...
const trimmedRequiredString = (label, maxLength) => v.pipe(
  v.string(),
  v.trim(),
...
const optionalTrimmedString = (maxLength) => v.optional(v.pipe(
  v.string(),
  v.trim(),
  v.maxLength(maxLength, `Field must be at most ${maxLength} characters long.`),
))

const optionalRawString = (maxLength) => v.optional(v.pipe(
  v.string(),
  v.maxLength(maxLength, `Field must be at most ${maxLength} characters long.`),
))

const positiveInteger = (label) => v.pipe(
  v.union([
    v.number(),
...
const numericId = (label) => v.pipe(
  v.string(),
  v.trim(),
  v.regex(/^\d+$/, `${label} must be a numeric string.`),
)

const ratingSchema = v.pipe(
  v.union([
    v.number(),
    v.pipe(v.string(), v.regex(/^\d+(\.\d+)?$/, 'rating must be numeric.')),
  ]),
  v.transform(Number),
...
export const registerBodySchema = v.pipe(v.looseObject({
...
}), v.check((input) => Boolean(input.email && input.password && input.role), 'email, password, and role are required.'))
...
export const refreshTokenBodySchema = v.pipe(v.looseObject({
  refresh_token: optionalTrimmedString(4096),
  refreshToken: optionalTrimmedString(4096),
}), v.check((input) => Boolean(input.refresh_token || input.refreshToken), 'refresh_token is required.'))

export const emailBodySchema = v.looseObject({
  email: v.pipe(v.string(), v.trim(), v.email('email must be a valid email address.')),
})

export const accountTokenBodySchema = v.pipe(v.looseObject({
  token: optionalTrimmedString(4096),
}), v.check((input) => Boolean(input.token), 'token is required.'))

export const passwordResetConfirmBodySchema = v.pipe(v.looseObject({
  token: optionalTrimmedString(4096),
  password: passwordSchema,
}), v.check((input) => Boolean(input.token && input.password), 'token and password are required.'))

export const userProfileUpdateBodySchema = v.pipe(v.looseObject({
  display_name: optionalTrimmedString(120),
  displayName: optionalTrimmedString(120),
}), v.check((input) => (
  input.display_name !== undefined || input.displayName !== undefined
), 'display_name is required.'))

export const passwordChangeBodySchema = v.pipe(v.looseObject({
...
}), v.check((input) => Boolean(input.current_password || input.currentPassword), 'current_password is required.'),
v.check((input) => Boolean(input.new_password || input.newPassword || input.password), 'new_password is required.'))

export const categoryBodySchema = v.pipe(v.looseObject({
...
export const teacherBodySchema = v.pipe(v.looseObject({
...
export const courseBodySchema = v.pipe(v.looseObject({
...
export const enrollmentBodySchema = v.pipe(v.looseObject({
