...
export async function syncCourseDailyStats() {
  ...
  const activeMap = new Map(activeCounts.map((entry) => [entry.courseId.toString(), entry._count._all]))
  const newMap = new Map(newCounts.map((entry) => [entry.courseId.toString(), entry._count._all]))

  await prisma.$transaction(courses.map((course) => prisma.courseDailyStat.upsert({
    where: {
      courseId_metricDate: {
        courseId: course.id,
        metricDate: start,
...
export function startCourseDailyStatsScheduler() {
  if (!env.ENABLE_DAILY_STATS_JOB || env.ENABLE_BACKGROUND_WORKERS || env.NODE_ENV === 'test' || intervalHandle) {
    return
  }
