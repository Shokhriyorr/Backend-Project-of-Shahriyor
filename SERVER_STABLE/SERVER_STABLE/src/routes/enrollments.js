...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const enrollments = await prisma.enrollment.findMany({
    where: {
      AND: [
        baseWhere,
        buildCreatedAtCursorFilter(req.query.cursor, direction, 'enrolledAt'),
      ],
    },
    orderBy: [
  ...
  return res.json(pageResponse(
    ...
    limit,
    serializeEnrollment,
    (record) => makeCreatedAtCursor(record, 'enrolledAt'),
  ))
}))

router.post('/', validateBody(enrollmentBodySchema), asyncHandler(async (req, res) => {
