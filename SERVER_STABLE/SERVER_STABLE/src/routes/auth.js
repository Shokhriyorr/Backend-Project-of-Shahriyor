...
router.post('/register', authRateLimiter, validateBody(registerBodySchema), asyncHandler(async (req, res) => {
  const email = req.body.email.trim().toLowerCase()
  const displayName = req.body.display_name ?? req.body.displayName ?? null

  if (req.body.role !== 'student') {
    throw new ApiError(403, 'forbidden', 'Public registration is limited to student accounts. Admin accounts must be provisioned by an existing operator.')
  }
  ...
  const emailJob = await enqueueEmail(buildVerificationEmail({
    user,
...
router.post('/login', authRateLimiter, validateBody(loginBodySchema), asyncHandler(async (req, res) => {
  const email = req.body.email.trim().toLowerCase()
...
router.post('/refresh', validateBody(refreshTokenBodySchema), asyncHandler(async (req, res) => {
  const refreshToken = req.body.refresh_token ?? req.body.refreshToken
  const payload = await refreshAccessToken(refreshToken, req)
  return res.json(payload)
}))

router.post('/logout', validateBody(refreshTokenBodySchema), asyncHandler(async (req, res) => {
  const refreshToken = req.body.refresh_token ?? req.body.refreshToken
  await revokeRefreshToken(refreshToken)
  return res.json(mutationSuccess('Refresh session revoked successfully.'))
}))

router.get('/verify-email', authRateLimiter, asyncHandler(async (req, res) => {
...
router.post('/verify-email', authRateLimiter, validateBody(accountTokenBodySchema), asyncHandler(async (req, res) => {
...
router.post('/resend-verification', authRateLimiter, validateBody(emailBodySchema), asyncHandler(async (req, res) => {
  const email = req.body.email.trim().toLowerCase()
  const user = await prisma.user.findUnique({
    where: { email },
  })
  ...
  if (user && !user.emailVerifiedAt) {
    ...
    await enqueueEmail(buildVerificationEmail({
      user,
      token: verificationToken,
    }))
  }

  return res.json(mutationSuccess('If the account exists and is unverified, a verification email has been queued.'))
}))

router.post('/password-reset/request', authRateLimiter, validateBody(emailBodySchema), asyncHandler(async (req, res) => {
  const email = req.body.email.trim().toLowerCase()
  const user = await prisma.user.findUnique({
    where: { email },
  })
  ...
  if (user) {
    ...
    await enqueueEmail(buildPasswordResetEmail({
      user,
      token: resetToken,
    }))
  }

  return res.json(mutationSuccess('If an account exists for this email, a password reset email has been queued.'))
}))

router.post('/password-reset/confirm', authRateLimiter, validateBody(passwordResetConfirmBodySchema), asyncHandler(async (req, res) => {
