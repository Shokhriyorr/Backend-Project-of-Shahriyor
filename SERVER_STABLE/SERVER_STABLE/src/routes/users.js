...
import { queueAccountUpdatedEmail, queuePasswordChangedEmail } from '../services/notificationService.js'
import { ApiError, asyncHandler } from '../utils/api.js'
import { serializeUser } from '../utils/serializers.js'
import { passwordChangeBodySchema, userProfileUpdateBodySchema } from '../validation/schemas.js'
...
router.patch('/me', requireAuth, validateBody(userProfileUpdateBodySchema), asyncHandler(async (req, res) => {
...
router.put('/me/password', requireAuth, validateBody(passwordChangeBodySchema), asyncHandler(async (req, res) => {
  const userId = BigInt(req.user.id)
  ...
  return res.json({
    ok: true,
    message: 'Password changed. Existing refresh sessions were revoked and a confirmation email has been queued.',
