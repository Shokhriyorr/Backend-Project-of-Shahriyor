...
router.post('/', requireAuth, requireRole('admin'), validateBody(teacherBodySchema), asyncHandler(async (req, res) => {
...
router.put('/:id', requireAuth, requireRole('admin'), validateBody(teacherBodySchema), asyncHandler(async (req, res) => {
...
router.delete('/:id', requireAuth, requireRole('admin'), asyncHandler(async (req, res) => {
  ...
  if (activeReference) {
    throw new ApiError(409, 'conflict', 'Cannot delete teacher while non-archived courses still reference it.')
  }
  ...
  try {
    ...
    if (error.code === 'P2003') {
      throw new ApiError(409, 'conflict', 'Cannot delete teacher while courses still reference it.')
    }
    throw error
  }
