...
import { queueEnrollmentCancelledEmail, queueEnrollmentCreatedEmail } from './notificationService.js'
    ...
    try {
      const enrollment = await prisma.$transaction(async (tx) => {
        ...
        if (seatUpdate.count !== 1) {
          throw new ApiError(409, 'conflict', 'Enrollment could not be completed because seat availability changed. Please retry.', {
            retryable: true,
          })
        }
...
export async function cancelEnrollment({ req, userId, courseId }) {
  ...
  throw new ApiError(409, 'conflict', 'Enrollment cancellation could not be completed after multiple retries.')
}
