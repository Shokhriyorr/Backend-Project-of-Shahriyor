...
import { syncCourseDailyStats } from '../../catalog/jobs/course-daily-stats.job.js'
import { getEmailQueueSnapshot } from '../../notifications/queues/email.queue.js'
import {
  enqueueCourseDailyStatsJob,
  getMaintenanceQueueSnapshot,
} from '../../operations/queues/maintenance.queue.js'
...
import { serializeAuditLog, serializeCourseDailyStat } from '../../../shared/utils/serializers.js'
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const auditLogs = await prisma.auditLog.findMany({
      where: {
        AND: [where, buildCreatedAtCursorFilter(req.query.cursor, 'desc')],
      },
...
router.post(
  ...
  asyncHandler(async (_req, res) => {
    ...
    return res.json(
      mutationSuccess(
        'Course daily stats synced synchronously because background workers are disabled.',
      ),
    )
  }),
)
